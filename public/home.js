...
function search(event){
    ...
    fetch("/search_content?q="+encodeURIComponent(form_data.get('search'))).then(searchResponse).then(jsonSpotify);
...
function saveSong(event){
  ...
  fetch("/save_song", {method: 'post', body: formData, headers: {'X-CSRF-TOKEN': token}}).then(dispatchResponse, dispatchError);
  event.stopPropagation();
}
